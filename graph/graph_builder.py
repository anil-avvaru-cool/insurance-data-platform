...
def build_graph_from_claims(
    ...
    shared_phone_pool = [faker.phone_number() for _ in range(300)]
    shared_address_pool = [faker.address().replace("\n", ", ") for _ in range(200)]

    fraud_phone_pool = shared_phone_pool
    fraud_address_pool = shared_address_pool

    fraud_attorney_pool = [f"fraud_attorney_{i}" for i in range(20)]
    legit_attorney_pool = [f"legit_attorney_{i}" for i in range(200)]
    ...
    with driver.session() as session:
        ...
        def _process_batch(tx, batch_df: pd.DataFrame) -> None:
            ...
            for _, claim in batch_df.iterrows():
                ...
                if is_fraud:
                    ...
                    attorney_id = (
                        ...
                        if (claim["attorney_present"] and random.random() < 0.15)
                        else (
                ...
                filed_rels.append({"claimant_id": claimant_id, "claim_id": claim_id})

                if attorney_id:
                    if attorney_id not in attorneys:
                        attorneys[attorney_id] = faker.name()
                    rep_by_rels.append({"claimant_id": claimant_id, "attorney_id": attorney_id})
                ...
                if vehicle_info is not None:
                    ...
                    involves_rels.append({"claim_id": claim_id, "vin": vin})
                else:
                    logger.warning("No resolved vehicle for quote_id %s — Vehicle node skipped", claim["quote_id"])
                ...
                for attr, raw_value in [("phone", claimant_phone), ("address", claimant_address)]:
                    ...
                    new_entities.append({"id": entity_id, "type": attr, "value": raw_value})
                    claimant_shares.append({"claimant_id": claimant_id, "entity_id": entity_id, "type": attr})
                    claim_shares.append({"claim_id": claim_id, "entity_id": entity_id, "type": attr})

            tx.run("""
                UNWIND $rows AS row
                MERGE (c:Claimant {id: row.id})
                SET c.name = row.name, c.phone = row.phone, c.address = row.address, c.is_fraud = row.is_fraud
                """, rows=claimants)

            tx.run("""
                UNWIND $rows AS row
                MERGE (cl:Claim {id: row.id})
                SET cl.is_fraud = row.is_fraud, cl.state = row.state
                """, rows=claims)

            tx.run("""
                UNWIND $rows AS row
                MATCH (c:Claimant {id: row.claimant_id}), (cl:Claim {id: row.claim_id})
                MERGE (c)-[:FILED]->(cl)
                """, rows=filed_rels)
            ...
            if attorneys:
                ...
                tx.run("""
                    UNWIND $rows AS row
                    MATCH (c:Claimant {id: row.claimant_id}), (a:Attorney {id: row.attorney_id})
            ...
            if vehicles:
                tx.run("""
                    ...
                    SET v.make = row.make, v.model = row.model, v.year = row.year
                    """, rows=[{"vin": vin, **info} for vin, info in vehicles.items()])
                tx.run("""
                    UNWIND $rows AS row
                    MATCH (cl:Claim {id: row.claim_id}), (v:Vehicle {vin: row.vin})
                    MERGE (cl)-[:INVOLVES]->(v)
                    """, rows=involves_rels)
            ...
            if new_entities:
                tx.run("""
                    ...
                    SET e.type = row.type, e.value = row.value
                    """, rows=new_entities)

            tx.run("""
                UNWIND $rows AS row
                MATCH (c:Claimant {id: row.claimant_id}), (e:Entity {id: row.entity_id})
                MERGE (c)-[:SHARES {type: row.type}]->(e)
                """, rows=claimant_shares)

            tx.run("""
                UNWIND $rows AS row
                MATCH (cl:Claim {id: row.claim_id}), (e:Entity {id: row.entity_id})
        ...
        for start in range(0, total, batch_size):
            ...
            logger.info("Importing claims %s..%s / %s", start + 1, end, total)
        ...
        logger.info("Entities shared across claims: %s", connected_entities)
